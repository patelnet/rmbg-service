...
void AppState::Initialize() {
    ...
    m_tray.Initialize(L"BackgroundRemover - idle");
    UpdateStatus(L"Idle");
}
...
bool AppState::StartWatching() {
    ...
    if (folder.empty() || !fs::exists(folder)) {
        ...
        m_tray.ShowBalloon(L"BackgroundRemover", L"Watch folder does not exist.", true);
        return false;
    }
    ...
    if (ok) {
        ...
        m_tray.SetTooltip(L"BackgroundRemover - watching");
    } else {
        UpdateStatus(L"Failed to start watching");
        Log(L"ReadDirectoryChangesW setup failed for " + folder);
        m_tray.ShowBalloon(L"BackgroundRemover", L"Failed to watch folder.", true);
    }
    return ok;
}
...
void AppState::StopWatching() {
    ...
    m_tray.SetTooltip(L"BackgroundRemover - idle");
}
...
void AppState::ProcessFileAsync(std::wstring fullPath) {
    ...
    std::thread([this, path = std::move(fullPath)] {
        ...
        if (result) {
            Log(L"Saved: " + *result);
            UpdateStatus(L"Done");
            m_tray.ShowBalloon(L"BackgroundRemover",
